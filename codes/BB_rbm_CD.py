...
def test_mnist_14x14(learning_rate=0.1,
                     training_epochs=15, 
                     dataset='../datasets/mnist_14x14.pkl.gz', 
                     batch_size=10, 
                     n_chains=20,
                     n_samples=10, 
                     output_folder='MNIST_14x14_rbm_CD_plots',
                     n_hidden=200):

    print 'Loading MNIST 14x14 dataset...'
    datasets = load_data(dataset)
    ...
    print 'Creating RBM...'
    rbm = RBM(input=x, n_visible=14 * 14,
              n_hidden=n_hidden, numpy_rng=rng, theano_rng=theano_rng)
    ...
    image_data = numpy.zeros(
        (15 * n_samples + 1, 15 * n_chains - 1),
        dtype='uint8'
    )
    for idx in xrange(n_samples):
        vis_mf, vis_sample = sample_fn()
        print ' ... plotting sample ', idx
        image_data[15 * idx:15 * idx + 14, :] = tile_raster_images(
            X=vis_mf,
            img_shape=(14, 14),
            tile_shape=(1, n_chains),
            tile_spacing=(1, 1)
