...
class RBM(object):
    ...
    def get_cost_updates(self, lr=0.1, mu=0.9, k=1):
	...
	for gparam, param in zip(gparams, self.params):
		    momentum_param = theano.shared(param.get_value()*0.)
		    v = mu * momentum_param - T.cast(lr,dtype=theano.config.floatX) * gparam
		    w = param + v
		    updates[param].append((momentum_param,v))
